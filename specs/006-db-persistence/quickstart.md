...
> 實作狀態:規劃中。以下指令名稱(`db:generate`/`db:migrate`/`db:seed`)為本 feature 將新增的 npm script。
...
- [ ] DbModule + DATABASE_URL config(fail fast)
- [ ] Drizzle schema + migration(accounts/decks/deck_revisions/themes)
- [ ] DbUserAccountStore 取代 ConfiguredUserAccountStore
- [ ] db:seed(AUTH_ACCOUNTS → accounts,idempotent)
- [ ] createDeckFromPreviewResult 純函式 + 測試
- [ ] worker 成功後 auto-save(持久化失敗只記錄不阻斷)
- [ ] DeckStore port + DrizzleDeckStore
- [ ] GET /api/decks、/api/decks/:id(JwtAuthGuard + 擁有權隔離)
- [ ] 前端最小「我的簡報」列表/檢視
- [ ] OpenAPI 文件補 decks 端點
