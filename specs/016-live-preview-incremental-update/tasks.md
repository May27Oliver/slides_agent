...
## Definition of Done & Anti-Drift（每階段結尾 + 收尾都套用）

> 防 legacy / shim / dead code / drift 的固定檢查。任一階段取代既有行為時必過。

**每階段結尾（取代既有時）**：
- [ ] 取代舊行為 → **改寫既有測試**，不新增平行測試；舊測試不保留。
- [ ] `grep` 確認被取代的舊符號/型別/i18n key/CSS class **0 references**（不留孤兒）。
- [ ] 無相容 shim、無「support both shapes」參數、無未走到的分支；舊路徑同次改動內刪除。

**Anti-Drift（同一事實多處出現時）**：
- [ ] 能「衍生」就不「複製」；非複製不可者（如 contracts validator + JSON schema +（未來）openapi），**加一個斷言三者一致的測試**。
- 本 feature 的 drift 點：`deck:patchSlides` 訊息形狀（contracts 協議文件 ↔ runtime handler ↔ LivePreview 送出）；`renderSlidesRegion` 輸出 ↔ `renderTemplateDeck` 投影片區（已由 T002 斷言相等）。

**收尾機械掃描（必跑）**：
- [ ] `pnpm -r exec tsc --noEmit` 全綠（已開 `noUnusedLocals`/`noUnusedParameters` → 未用 local/param/import 直接紅燈）。
- [ ] `gitnexus_detect_changes()`：影響面只落在預期符號、無孤兒、無非預期擴散。
- [ ] 既有 domain 渲染測試全綠（= `renderTemplateDeck` html 未變的護欄）。
