...
**Status**: Ready for planning（兩輪 clarify 已定案；可進 /speckit.plan）
...
| 文字樣式覆寫範圍 | **標題 + message + 每條 outline 條列**。大小用預設級距（S/M/L/XL），顏色限定**主題色盤 token**（不開放任意 hex），維持與主題一致、資料模型簡單。 |
| 左側預覽比例 | **單張預覽固定 16:9 letterbox**：維持現有單張即時預覽，但把 iframe 框成 16:9（留邊），不再被容器拉伸變形。 |
| 下載內容來源 | **必須先存檔，下載目前畫面對應且 dirty=false 的「具體 revision number」**（非模糊的「server 最新」）。PPTX 請求帶該 revision number，後端以 id 驗證只匯出該版本（見第三輪 clarify 1）。 |
...
2. **PPTX 範圍 = 015 全做，含 PPTX**。四個 US 都納入本 feature。預設正式機 EC2 可裝/可用 chromium（或採內含 chromium 的 docker image）。
   → plan 需含：無頭瀏覽器依賴選型與打包、worker 端逐頁截圖→pptx 組裝、以及正式機（EC2 / docker-compose）的部署驗證步驟。

3. **顏色 token 集合 = 四種主題角色色**：`預設文字 / 強調(accent) / 次要(muted) / 標題色`，對應既有 CSS 變數（`--text` / `--accent` / `--muted` / heading 色），換主題仍一致。
   → contracts 的 `colorToken` 列舉固定為這四個值。

4. **大小級距 = S/M/L/XL，倍率 0.85 / 1 / 1.25 / 1.6×**，相對主題基準字級縮放；M 為主題預設（即「未設定」等同 M）。
   → contracts 的 `sizeLevel` 列舉固定為 `S/M/L/XL`；渲染以倍率套用在對應欄位的基準字級上（透過 inline style 或 CSS 變數）。
...
1. **下載目標 = 目前畫面對應且 dirty=false 的「具體 revision number」**（修正 finding 1）。下載不依賴模糊的「server 最新」：前端以目前 adopt 的 `revision`（`DeckRevisionContract.revision`）為下載對象，且僅在 `dirty=false` 時可下載。PPTX 匯出請求 MUST 帶上該 `revision` number，**後端以 revision id 驗證並只匯出該版本**，避免匯出到使用者沒看到的版本（含其他 tab 造成的 latest）。若請求的 revision 已非存在版本，回明確錯誤。
...
4. **TextStyleOverride 資料形狀鎖定**（修正 finding 4）。附掛於 slide 的 `textStyleOverrides`：
   ...
   TextStyleOverride = { sizeLevel?: 'S'|'M'|'L'|'XL'; colorToken?: 'text'|'accent'|'muted'|'heading' }
   ```
   - **預設不儲存**：size=M 與 color=預設(text) 視為「未設定」→ 不寫入（省欄位、避免噪音）；某欄位兩屬性皆預設時整個 entry 省略。
   - **reset 粒度**：UI 同時支援「單一屬性 reset」（清掉 size 或 color 其一）與「整欄 reset」（清掉該欄位整個 entry）。
...
**Independent Test**: 在編輯頁點某欄位（標題/message/某條列）開啟樣式工具列，調整大小級距與顏色，即時預覽更新；Save 後重載，覆寫保留並反映在 server 渲染 html。

**Independent Demo**: 點標題 → 選 XL + 強調色 → 預覽即時變化 → Save → 重新整理仍在。

**Acceptance Scenarios**:

1. **Given** 編輯頁選定一張 slide，**When** 使用者點擊標題欄位的樣式控制，選擇大小級距（S/M/L/XL 其一）與主題色盤中的一個顏色，**Then** 左側即時預覽中該 slide 的標題以對應大小與顏色呈現。
...
5. **Given** 顏色只能選主題色盤 token，**When** 使用者操作顏色控制，**Then** 介面只提供有限的主題色 swatch，無自由 hex 輸入（維持主題一致性）。
...
- **主題切換後的顏色 token**：顏色覆寫存的是色盤 token 而非固定色值，換主題後仍對應該主題的對應角色色。
- **覆寫值越界**：大小與顏色皆為列舉值，超出列舉一律拒絕（天然 DoS 邊界）。
...
- **FR-003a**: 後端 MUST 以請求帶入的 `revision` number 驗證並**只匯出該版本**；若該 revision 不存在或不屬於該帳號的 deck，回明確錯誤，不退而匯出其他版本。
...
- **FR-007**: 編輯頁 MUST 允許針對選定 slide 的標題、message、以及每一條 outline 條列，各自設定文字大小覆寫，大小為固定級距列舉 `S/M/L/XL`（倍率 0.85 / 1 / 1.25 / 1.6×，相對主題基準字級；M = 主題預設）。
- **FR-008**: 編輯頁 MUST 允許上述同範圍欄位設定文字顏色覆寫，顏色僅能選自四個主題角色色 token：`預設文字 / 強調 / 次要 / 標題色`（對應 `--text` / `--accent` / `--muted` / heading 色），不提供任意 hex。
...
- **FR-016**: 文字樣式覆寫 MUST 以鎖定結構儲存：slide 上的 `textStyleOverrides?: { title?, message?, outlineById?: Record<outlineItemId, TextStyleOverride> }`，`TextStyleOverride = { sizeLevel?: S|M|L|XL; colorToken?: text|accent|muted|heading }`。預設值（size=M、color=text）視為未設定不寫入；兩屬性皆預設時整個 entry 省略。
...
- **FR-013**: 文字樣式覆寫的所有輸入 MUST 在 contracts 邊界以列舉/白名單驗證，越界值一律拒絕；server 端 edit 合併 MUST 將覆寫納入既有 revision 合併白名單，且不破壞 contentBlocks 既有唯讀規則。
...
- **CR-005 Design System**: 文字樣式覆寫 MUST 在既有 design system 邊界內運作——大小為主題基準字級的相對級距、顏色為主題色盤 token，避免破壞既有 palette / typography / spacing / 視覺密度與可重用版型。
...
- **CR-009 Domain Model**: 新增領域概念「文字樣式覆寫（TextStyleOverride：sizeLevel 列舉 + colorToken 列舉）」附掛於 slide 的標題/message/outline 條列；以及「PPTX 匯出工作」狀態（沿用既有 preview-job 佇列模型語彙）。
...
- **CR-012 Code Simplicity**: 範圍邊界明確——大小/顏色皆為有限列舉（不做任意數值/hex）、PPTX 採截圖（不做原生形狀對映）、預覽比例為純前端 letterbox、下載只對已存 revision；避免投機抽象。
- **CR-013 Consistent Language**: 關鍵詞需跨 UI/報告/文件一致：下載 HTML、下載 PPTX、文字樣式（大小/顏色）、重設樣式、匯出工作、16:9 預覽。
...
- **TextStyleOverride（文字樣式覆寫）**: 附掛於某 slide 之某欄位（標題 / message / 某 outline 條列「以穩定 id 綁定」）的呈現覆寫。屬性：`sizeLevel`（列舉 `S/M/L/XL`，倍率 0.85/1/1.25/1.6×，M=預設）、`colorToken`（列舉，四值：預設文字 / 強調 / 次要 / 標題色）。皆為選填；未設定即沿用主題預設（等同 size=M、color=預設角色色）。隸屬於既有 Slide 領域模型，隨 slideDeck 經 edit revision 持久化。
...
- **SC-005**: 所有樣式覆寫與匯出參數皆通過 contracts 邊界的列舉/白名單驗證；越界輸入 100% 被拒絕，無 DoS 放大路徑。
...
- 文字顏色限定主題色盤 token、大小限定固定級距為可接受表達範圍（已定案決策）。
