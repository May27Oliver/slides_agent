...
在既有編輯頁（010/011/014）上加四件事：US1 下載 HTML（純前端複用 helper）、US4 左側預覽固定 16:9（純前端 CSS）、US3 逐欄位文字大小（px）/顏色（hex）/字型覆寫（domain 型別 + 單一 renderer 套用 + 編輯 UI）、US2 下載 PPTX（鏡射 preview-jobs 的非同步工作 + worker 無頭 chromium 截圖 + pptxgenjs）。技術路線見 [research.md](./research.md)、型別見 [data-model.md](./data-model.md)、契約見 [contracts/](./contracts/)。
...
**Constraints**: 樣式覆寫輸入以「數值範圍（sizePx 8–240）＋ hex regex（color）＋ 字型白名單/長度（fontFamily ≤64）＋ outlineById ≤100 entries」為 bounded DoS 邊界；PPTX 跨帳號隔離、無殘檔、TTL 清理；PPTX 單人併發=1 以 store 原子 `SET NX` 鎖（create-if-no-active，無 TOCTOU）強制。
...
├── SlideEditPanel.tsx                         # 右側滑出樣式面板（px 滑桿 + 自由色彩選擇器 + 字型下拉 + reset）
...
- **C1 domain（TDD 先行）**：型別（data-model §1,§2）；`text-style-override.ts` 純函式（sizePx/color/fontFamily→inline style）；`template-html-renderer.ts` 三點注入 + 用到的字型注入 Google Fonts `<link>`；renderer 測試（套用後 html 含正確 `font-size:<px>px` / `color:<#RRGGBB>` / `font-family:'<family>'`）。
...
- **C4 web**：`editable-slide-draft` 惰性補 id + setTitleStyle/setMessageStyle/setOutlineStyle/reset + 刪條列清孤兒；`SlideEditPanel` 右側滑出樣式面板（px 滑桿 + 自由色彩選擇器 + 字型家族下拉 + 單屬性/整欄 reset）；i18n 詞彙。
...
- **D2 API 側**：`pptx-export-jobs` module 鏡射 preview-jobs（store/runner/queue/controller/parser/sweeper/runtime/tokens/providers/module）；三端點 + 建立驗證（revision 仍為 current＝current-only、頁數≤60、單人併發=1 以 store 原子 `createIfNoActive`/`SET NX`、owner scope）；掛 `app.module.ts`。
