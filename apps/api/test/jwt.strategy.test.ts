...
describe("JwtStrategy", () => {
  it("returns the user and token expiry for a valid active account", async () => {
    const strategy = strategyWith(
      vi.fn().mockResolvedValue({ id: "u", username: "a", displayName: "A" })
    );

    const result = await strategy.validate({ sub: "u", exp: 1_893_456_000 });

    expect(result).toMatchObject({ id: "u", username: "a", displayName: "A" });
